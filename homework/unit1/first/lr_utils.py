...
#加载数据
def load_dataset():
    train_dataset = h5py.File('datasets/train_catvnoncat.h5', "r") # <class 'h5py._hl.files.File'>
...
if __name__ == '__main__':
    ...
    m_test = train_set_y_orig.shape[1]  # 测试集里图片的数量。
    ...
    # print("测试集_标签的维数: " + str(train_set_y_orig.shape))

    # 将训练集的维度降低并转置。
    train_set_x_flatten = train_set_x_orig.reshape(train_set_x_orig.shape[0], -1).T # (12288, 209) (n,m)
    # 将测试集的维度降低并转置。
    test_set_x_flatten = test_set_x_orig.reshape(test_set_x_orig.shape[0], -1).T # (12288, 50) (n,m)
...
# #绘制图
# costs = np.squeeze(d['costs']) # 压缩维度，将costs转变为秩为1的数组
# plt.plot(costs)
# plt.ylabel('cost')
...
#     models[str(i)] = model(traimodeln_set_x, train_set_y_orig, test_set_x, test_set_y_orig,
...
#
# legend = plt.legend(loc='upper center', shadow=True) # 设置图例背景为灰色
# frame = legend.get_frame() # 获得图例背景
# frame.set_facecolor('0.90') # 设置图例背景透明度
# plt.show()
